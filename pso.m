...
%  
%  xt=[-10 -10 -7.5 -10  0 -5 ];
%  yt=[25 27.5 27.5 30 30 30 ];

%case 3
% xs=[30 25 30 30 25 25];
% ys=[-5 0 0 -2.5 -2.5 -5];
% 
% xt=[-15 -15 -15 -12.5 -5 -10];
% yt=[25 20 30 25 20 30 ];
% staring psition     
xs=-30;
ys= -30;
%target position
...
%  xs=[-30 -30  -25 -25 -25 -20 -20];
% ys=[-30 -25 -22.5 -30 -25 -30 -25];
% xt=[30 30 25 25 25 20 20];
% yt=[30 25 22.5 30 25 30 25 ]; 
%     
    for k=1:4
%% Problem Definition
...
c2=2.2;             % Global Learning Coefficient
%TVAC
...
%      D = H;
%      F_G = (b1)./(1+exp(-a1*(G-c))) +d;   
%      F_H = (b1)./(1+exp(-a1*(H-c))) +d;   
%      c1 = (F_G);
%      c2 = (F_H);
% % HTPSO
% G =   particle(i).Best.Position.x-particle(i).Position.x;
...
% c2 = (F_H);
  ...
  %F_R = abs(GlobalBest.Position.x-particle(i).Position.x)*((it)./MaxIt);
  %w = (wf./(wi+(exp(-(0.5)*F_R))));

%  G =    particle(i).Best.Position.x-particle(i).Position.x;
%  H =  GlobalBest.Position.x-particle(i).Position.x;
%  D = particle(i).Best.Position.x -  GlobalBest.Position.x;
%      
%  F_G = (2*(b)./(1+exp(2*(-a*(G-c))))-1) +1.35; 
%  F_H = ((2*(b)./(1+exp(2*(-a*(H-c))))-1)) + 1.35;
%  D_H = ((2*(b)./(1+exp(2*(-a*(H-c))))-1)) + 1.35;
%  c1 = (F_G);
%  c2 = (F_H);
%  c3 = (D_H);
%  + c3.*rand(VarSize).*(particle(i).Best.Position.x-GlobalBest.Position.x)
%         
        % Update Velocity
